...
function normalizeTestData(data) {
    ...
    if (Array.isArray(data.q)) {
        ...
        title = data.t;
        minutes = data.m;
        negativeMarking = Number(data.n) || 0;
        shuffle = !!data.s;
...
// ============================================================
//
// Used to derive a short, stable sessionStorage key from a
// (potentially very long) test token, so "resume this attempt"
// state doesn't need to store the whole token as a key.
// ============================================================
...
export async function encodeTest(obj) {
    ...
    if (obj.negativeMarking) {
        compact.n = obj.negativeMarking;
    }
    if (obj.shuffle) {
        compact.s = 1;
    }
